      ...
      <div className={`px-4 py-3 rounded-xl max-w-lg shadow-sm ${isUser ? 'bg-primary text-white rounded-br-none' : 'bg-white text-neutral-800 rounded-bl-none'}`}>
        ...
        {!isUser && (
          <div className="mt-2 pt-2 border-t border-neutral-200/50 flex items-center justify-end space-x-2">
            ...
            <button onClick={() => onFeedback(message.id, 'good')} className="p-1 rounded-full hover:bg-emerald-100 text-neutral-500 hover:text-emerald-600 transition-colors">
            ...
            <button onClick={() => onFeedback(message.id, 'bad')} className="p-1 rounded-full hover:bg-red-100 text-neutral-500 hover:text-red-600 transition-colors">
