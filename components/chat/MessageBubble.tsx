  ...
  return (
    <div className={`flex items-start gap-2 px-4 ${isUser ? 'justify-end' : 'w-full justify-start'}`}>
      ...
      <div className={`px-4 py-3 ${isUser
        ? 'max-w-lg shadow-sm bg-primary text-white rounded-xl rounded-tr-none'
        : 'w-full bg-transparent text-text'
        }`}>
        <div className={`prose prose-sm max-w-none break-words [&_p]:mb-6 ${isUser ? 'prose-invert text-white' : 'text-text prose-headings:text-text prose-p:text-text prose-strong:text-text prose-ul:text-text prose-ol:text-text'}`}>
          <ReactMarkdown
            components={{
        ...
        {!isUser && (
          <div className="mt-2 pt-2 border-t border-border flex items-center justify-end space-x-2">
            <span className="text-xs text-text-muted">Helpful?</span>
            <button
              onClick={() => onFeedback(message.id, 'good')}
              className="p-1 rounded-full hover:bg-primary/10 text-text-muted hover:text-primary transition-colors"
