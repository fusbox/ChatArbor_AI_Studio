...
const Header: React.FC<HeaderProps> = ({ title, currentView, onViewChange, onAuthClick }) => {
  ...
  return (
    <header className="sticky top-0 bg-primary-dark text-white shadow-md p-4 flex justify-between items-center z-20">
      ...
      {currentUser ? (
        <div className="flex items-center space-x-4">
          <nav className="bg-primary-dark/50 p-1 rounded-lg flex space-x-1">
            <NavButton label="Chat" isActive={currentView === 'chat'} onClick={() => onViewChange('chat')} data-testid="nav-chat" />
            <NavButton label="Admin Panel" isActive={currentView === 'admin'} onClick={() => onViewChange('admin')} data-testid="nav-admin-panel" />
          </nav>
