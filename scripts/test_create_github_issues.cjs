#!/usr/bin/env node
// Test helper (CommonJS): checks which issues from `.github/ISSUES/*.md` are missing in the repo.
// Usage: set GITHUB_TOKEN env var and run:
//   node scripts/test_create_github_issues.cjs --repo fusbox/ChatArbor_AI_Studio
...
(async function main() {
  ...
  // Helper: normalize titles similarly to the PowerShell seeder
  function normalizeTitle(t) {
    if (!t) return '';
    return t.replace(/[\u2014\u2013]/g, ' - ').replace(/[\u2018\u2019]/g, "'").replace(/[\u201C\u201D]/g, '"').replace(/&/g, ' and ').replace(/\s+/g, ' ').trim().toLowerCase();
  }
  ...
  const missing = files.filter(f => !titles.has(normalizeTitle(f.title)));
  if (missing.length === 0) {
